...
# else
    if [ "$POWERLINE_SHOW_GIT_ON_RIGHT" = "" ]; then
        if [ "$POWERLINE_HIDE_GIT_PROMPT_STATUS" = "" ]; then
            POWERLINE_GIT_INFO_LEFT=" %F{blue}%K{green}"$'\ue0b0'"%F{green}%F{black}%K{green}"$'$(git_prompt_info)$(git_prompt_status)%F{green}'
        else
            POWERLINE_GIT_INFO_LEFT=" %F{blue}%K{green}"$'\ue0b0'"%F{green}%F{black}%K{green}"$'$(git_prompt_info)%F{green}'
        fi
        POWERLINE_GIT_INFO_RIGHT=""
    else
        POWERLINE_GIT_INFO_LEFT=""
